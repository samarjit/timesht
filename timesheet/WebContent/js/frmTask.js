...
//Used for inserting new record from details screen
function newInsert(){
	screenMode = "create";
	jQuery('span#screenMode').text(screenMode);
	//enabling
	updateonar = "btnSave,project,taskdesc,tskid,tskhours,tskpercentcpl,tskstartdate,tskenddate,btnSave".split(",");
	for ( var i = 0; i < updateonar.length; i++) {
		var arelm = updateonar[i];
		jQuery("#"+ arelm).removeAttr('disabled');
	}
...
function reqSave() {
	...
	//alert("in savesdkgf ");	
	//var url=urlpart+"?panelName=searchPanel&screenName=frmTask"+screenName;	
	 
	if(screenMode == "create"){
	//	document.getElementById("qtyavailable").value = document.getElementById("quantity").value;		
	//document.getElementById("tskid").value = "AUTOGEN_SEQUENCE_ID";	
	var url=inserturlpart+"?panelName=searchPanel&screenName=frmTask";
	//prompt("url","action=true&doString="+actionid+"&wflid="+wflid+"&appid="+applicationid);	
	url = url+ "&insertKeyValue="+ prepareInsertData()+"&invokewfl=false&activityname=CRTASK&action=true";
	//prompt("url",url);
	...
	if(screenMode == "modify"){
		...
		var url=updateurlpart+"?wclause="+whereclause+"&screenName=frmTask";
		//prompt("url",url);	
...
function deleteData(){
	...
	whereclause  = makeWhereClause();
	var url=deleteurlpart+"?wclause="+whereclause+"&screenName=frmTask";
//	prompt("url",url);	
...
function updateData(obj){
	//obj.disabled = true;
	screenMode = "modify";
	jQuery('span#screenMode').text("modify");
...
//dnid,hidspace,assetid,assetname,assettype,vendorid,make,tag,config,allocstatus,warranty,quantity,remarks,
	...
	//jQuery("#panelFields input[type='text']").each(function(){console.log(this.id);}) to find all input elements
var updateonar = "tskhours,project,taskdesc,tskpercentcpl,tskstartdate,tskenddate,btnSave".split(",");
for ( var i = 0; i < updateonar.length; i++) {
	var arelm = updateonar[i];
	jQuery("#" + arelm).removeAttr('disabled');
}
